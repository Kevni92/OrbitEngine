...
struct NumericalWire {
  ...
  std::uint32_t maneuver_stage_index = 0;
  time::TimeWire maneuver_stage_start{};
  time::TimeWire maneuver_stage_end{};
  double maneuver_force_magnitude_newtons = 0.0;
  double maneuver_mass_flow_kilograms_per_second = 0.0;
  bool maneuver_minimum_mass_present = false;
  double maneuver_minimum_mass_kilograms = 0.0;
  std::uint16_t maneuver_direction_kind = 0;
  std::uint32_t maneuver_direction_frame_high = 0;
  std::uint32_t maneuver_direction_frame_low = 0;
  std::uint32_t maneuver_direction_frame_revision_high = 0;
  std::uint32_t maneuver_direction_frame_revision_low = 0;
  double maneuver_direction_x = 0.0;
  double maneuver_direction_y = 0.0;
  double maneuver_direction_z = 0.0;
  std::uint32_t maneuver_attitude_source_high = 0;
  std::uint32_t maneuver_attitude_source_low = 0;
  std::uint32_t maneuver_attitude_revision_high = 0;
  std::uint32_t maneuver_attitude_revision_low = 0;
