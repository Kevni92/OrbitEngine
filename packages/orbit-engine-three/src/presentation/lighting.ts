...
export function inspectionFillContribution(mode: LightingMode, requestedStrength = DEFAULT_INSPECTION_FILL_STRENGTH): number {
  if (!Number.isFinite(requestedStrength) || requestedStrength < 0) throw new RangeError("inspection fill strength must be finite and non-negative");
  return mode === "enhanced" ? Math.min(MAX_INSPECTION_FILL_CONTRIBUTION, requestedStrength) : 0;
}

export function lightingModeDiagnostics(mode: LightingMode, targetObjectIds: Iterable<ObjectId>, requestedStrength = DEFAULT_INSPECTION_FILL_STRENGTH): LightingModeDiagnostics {
