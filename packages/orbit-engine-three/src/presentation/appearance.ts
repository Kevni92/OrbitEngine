...
/** Explicit, renderer-neutral optical calibration supplied by a scenario. */
export interface AtmosphereOpticalCalibration {
...
export interface AtmosphereAppearance {
  ...
  readonly optics?: AtmosphereOpticalCalibration;
...
export type StellarEmission = StellarEmissionAppearance;
...
/** Versioned deterministic fallback library, independent of any renderer. */
export const OPTICAL_LIBRARY_VERSION = "orbit-engine-three-optics-1" as const;
...
function validateOptics(path: string, optics: AtmosphereOpticalCalibration): void {
...
export function validateCelestialAppearance(appearance: CelestialAppearance | undefined, objectId?: ObjectId): void {
  if (appearance === undefined) return;
  const prefix = objectId === undefined ? "record" : `record for ${objectId}`;
  if (appearance.schemaVersion !== CELESTIAL_APPEARANCE_SCHEMA_VERSION) fail(`${prefix}.schemaVersion`, `unsupported version ${String(appearance.schemaVersion)}`);
  if (appearance.visibleLayer !== undefined) {
    if (!["solidSurface", "iceSurface", "cloudDeck"].includes(appearance.visibleLayer.kind)) fail(`${prefix}.visibleLayer.kind`, `unsupported layer kind ${String(appearance.visibleLayer.kind)}`);
  ...
  if (appearance.stellarEmission !== undefined) {
    ...
    if (appearance.stellarEmission.effectiveTemperatureKelvin < STELLAR_TEMPERATURE_RANGE_KELVIN.min || appearance.stellarEmission.effectiveTemperatureKelvin > STELLAR_TEMPERATURE_RANGE_KELVIN.max) {
...
export function createCelestialAppearance(input: CelestialAppearance): CelestialAppearance {
  ...
  const provenance = Object.freeze(input.provenance.map((source) => Object.freeze({ ...source, fields: Object.freeze([...source.fields]) })));
