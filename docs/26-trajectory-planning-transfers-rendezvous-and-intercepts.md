...
### Implemented TypeScript boundary (#164, #165, #166, #167)
...
Bounded transfer-window search is implemented with normalized exact-grid sampling over departure/arrival or departure/time-of-flight domains, deterministic physical ranking/tie-breaking, bounded coordinate refinement, candidate limits, optional derived grid samples, and explicit `completed`, `budgetExceeded` and `cancelled` diagnostics. Search cancellation is checked at bounded evaluation batches and remains non-authoritative. Search calls the same moving-object planning path as single transfers, so validity, frame, dependency and constraint semantics cannot diverge between a direct plan and a sampled candidate.
