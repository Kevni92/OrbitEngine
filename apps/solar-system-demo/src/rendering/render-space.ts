...
/** Converts authoritative ICRS-aligned directions into the demo's render axes. */
export function icrsDirectionToRenderSpace(direction: Vec3<number>): RenderVector {
  ...
  return Object.freeze({ x: rotated.x / length, y: rotated.y / length, z: rotated.z / length });
}
...
export function metersToSceneUnits(value: number): number { return finite(value, "meters") * METERS_TO_SCENE_UNITS; }
...
export function positionToSceneUnits(position: Vec3<number>, focus: Vec3<number> = { x: 0, y: 0, z: 0 }): RenderVector {
  ...
  return Object.freeze({ x: metersToSceneUnits(presentation.x), y: metersToSceneUnits(presentation.y), z: metersToSceneUnits(presentation.z) });
}
...
export function radiusToSceneUnits(policy: RadiusPolicy): number {
  ...
  if (policy.adaptiveRadiusSceneUnits === undefined) throw new RangeError("Adaptive radius requires a camera-aware scene-unit value");
